<div id="coinModal-{{ $transfer->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-between mb-4 rounded-t sm:mb-5">
                <div class="text-lg text-gray-900 md:text-xl dark:text-white">
                    ...
                    <p class="text-base">
                        Account Name - {{ $transfer->accname }}
                    </p>
                    <p class="text-base">
                        Amount - {{ $transfer->ggcoin * 1000 }} mmk
                    </p>
                    <p class="text-base">
                        Coins - {{ $transfer->ggcoin }} coins
                    </p>
                    <p class="text-base">
                        Payment Method - {{ $transfer->payment }}
                    </p>
                </div>
                <div>
                    <button type="button"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 inline-flex dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-toggle="coinModal-{{ $transfer->id }}">
            ...
            <div>
                ...
                <div class="mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-400 ">
                    <img src="{{ $transfer->image }}" alt="" class="max-h-[20rem]">
                </div>
            </div>
            <div class="flex items-center justify-end gap-2">
                <form action="/admin/users/coins/{{ $transfer->id }}/confirm" method="POST">
                    @csrf
                    <button type="submit"
                ...
                <form action="/admin/users/coins/{{ $transfer->id }}/delete" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
