                ...
                @foreach ($books as $book)
                    @if (!$book->isArchive)
                        <div class="">
                            ...
                            <!-- for image -->
                            <div class="relative">
                                <img class="h-[250px] w-[90%] rounded-md object-cover object-center"
                                    src="{{ $book->image }}" alt="" />
                                <div
                                    ...
                                    <ul class="flex items-center flex-col gap-5">
                                        ...
                                        @if ($current)
                                            <li class="border border-gray-200 w-full flex justify-center px-2"><a
                                                    href="/book/chapter/{{ $current->slug }}/read">continue reading</a>
                                            </li>
                                        @endif
                                    </ul>
                            ...
                            <div class="py-[10px] flex items-center gap-2">
                                <div class="w-[90%] bg-gray-300 rounded-full h-2.5 dark:bg-gray-700">
                                    <div class="bg-green-400 h-2.5 rounded-full" style="width: {{ $percentage }}">
                                    </div>

                                </div>

                                @if (!$current)
                                    <span><i class="fas fa-check-circle text-green-400"></i></span>
                                @endif
                            </div>
                            <!-- Progress bar end -->
