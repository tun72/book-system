        ...
        <div class="flex items-center justify-center mb-10 px-10">
            ...
            @if (auth()->user()?->isBought($book) || auth()->user()->role === 1 || auth()->user()->isAuthorBook($book))
                @if ($nextChapter)
                    <div class="wrapper">
                        <a class="cta"
                            href="/book/chapter/{{ $nextChapter->slug }}/read?complete={{ !auth()->user()->isAlreadyRead($chapter) ? "true" : "false" }}">
                            <span>NEXT</span>
                            <span>
                ...
                @if ($complete)
                    <div class="wrapper">
                        <a class="cta" href="/book/chapter/{{ $chapter->slug }}/complete">
                            <span>Complete</span>
                            <span>
                                <svg width="40px" height="30px" viewBox="0 0 66 43" version="1.1"
                                    xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    <div id="drawer-right-example"
        ...
        <ul class="flex flex-col gap-2">
            @foreach ($book->chapters as $chap)
                {{-- @dd(auth()->user()->isAlreadyRead($chap)); --}}
                <a
                    href="{{ auth()->user()->isBought($book) || $chap->is_free ? '/book/chapter/' . $chap->slug . '/read' : '#' }}">


                    <li class=" hover:bg-slate-200 cursor-pointer rounded-md  {{ $chap->id === $chapter->id ? 'bg-slate-200' : '#' }} {{ auth()->user()->isAlreadyRead($chap) ? 'text-green-500' : '' }} "
                        aria-selected="true">
                        <div class="flex items-center p-1 justify-between">
                            ...
                            <span class="inline-flex items-center text-sm font-semibold text-gray-900 dark:text-white">
                                @if (auth()->user()->isAlreadyRead($chap))
